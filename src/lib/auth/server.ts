...
const mysqlCfgEarly = mysqlConfigFromEnv();
const databaseUrlEarly = env("DATABASE_URL");
const hostedDeploy = Boolean(mysqlCfgEarly || databaseUrlEarly);
const grokClientId = env("GROK_AUTH_CLIENT_ID") ?? (hostedDeploy ? undefined : PREVIEW_CLIENT_ID);
const grokClientSecret = env("GROK_AUTH_CLIENT_SECRET") ?? (hostedDeploy ? undefined : PREVIEW_CLIENT_SECRET);

/** True when real auth is enforced (email/password and/or federated sign-in). */
export const authConfigured = !authDisabled && (emailAndPasswordEnabled || Boolean(grokClientId && grokClientSecret));
